...
export async function handleRepositoryDeleteMessage(
  ...
  // Step 0: fence every writer through the repository authority. Existing
  // receive, compaction, and snapshot-materialization leases must drain before
  // deletion mutates any persistent surface; new work is rejected once the
  // fence is set.
  const stub = getRepoStub(env, body.doName);
  let deletion: BeginRepositoryDeletionResult;
  try {
    if (!countSubrequest(cacheCtx, 1)) {
      log.warn("repo-delete:budget-exhausted", { step: "do-fence" });
      retryQueueMessage(message, DELETE_RETRY_DELAY_SECONDS);
      return;
    }
    deletion = await limiter.run<BeginRepositoryDeletionResult>("do:repo-delete-fence", () =>
      stub.beginRepositoryDeletion()
    );
  } catch (error) {
    log.warn("repo-delete:retry", { step: "do-fence", error: String(error) });
    retryQueueMessage(message, DELETE_RETRY_DELAY_SECONDS);
    return;
  }
  if (!deletion.ready) {
    log.info("repo-delete:waiting-for-writers");
    retryQueueMessage(message, DELETE_RETRY_DELAY_SECONDS);
    return;
  }
  if (
    deletion.snapshotPrefixes.some(
      (prefix) => !isRepositorySnapshotPrefix(prefix, body.repositoryId)
    )
  ) {
    log.warn("repo-delete:retry", { step: "snapshot-prefix-validation" });
    retryQueueMessage(message, DELETE_RETRY_DELAY_SECONDS);
    return;
  }
  ...
  // Step 3: R2 enumerate + delete the canonical Git storage and every derived
  ...
  for (const prefix of snapshotPrefixes) {
    prefixes.push({ surface: "snapshot", prefix });
  }
  ...
  try {
    for (const target of prefixes) {
      ...
      do {
        if (!countSubrequest(cacheCtx, 1)) {
          log.warn("repo-delete:budget-exhausted", {
            step: "r2-list",
        ...
        const listing = await limiter.run("r2:repo-delete-list", () =>
          env.REPO_BUCKET.list({ prefix: target.prefix, cursor })
        );
